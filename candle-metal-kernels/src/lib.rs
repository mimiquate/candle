...
macro_rules! ops{
    ($($name:ident),+) => {
        ...
        pub mod $name {
            ...
            pub const INT64: Kernel = Kernel(concat!(stringify!($name), "_i64"));
        }
        )+
            pub mod copy {
                use super::Kernel;
        ...
        pub mod $name {
            ...
            pub const INT64: Kernel = Kernel(concat!(stringify!($name), "_i64_strided"));
        }
        )+
            pub mod copy {
                use super::Kernel;
...
pub mod unary {
    ops!(cos, sin, exp, sqr, sqrt, neg, log, gelu, abs, ceil, floor, round, erf, gelu_erf, tanh);
}
pub mod binary {
